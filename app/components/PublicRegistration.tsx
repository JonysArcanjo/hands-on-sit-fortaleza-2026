  ...
  return <main className="public-shell">
    ...
    <section className="hero" id="top">
      ...
      <div className="registration-panel">
        ...
        {registration && <><span className="confirmation-mark" aria-hidden="true">✓</span><span className="eyebrow">INSCRIÇÃO CONFIRMADA</span><h2>Seu Hands-on está reservado.</h2><div className="confirmation-card"><h3>{registration.title}</h3><p>{registration.instructor}</p><p>{new Intl.DateTimeFormat("pt-BR", { dateStyle: "long", timeStyle: "short", timeZone: "America/Fortaleza" }).format(new Date(registration.startsAt))}</p><b>{registration.room}</b></div><button className="text-button" onClick={() => { setParticipant(null); setRegistration(null); setEmail(""); }}>Consultar outro e-mail</button></>}
