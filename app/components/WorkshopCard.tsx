...
export function WorkshopCard({ workshop, selected, onSelect }: { workshop: Workshop; selected: boolean; onSelect: () => void }) {
  ...
  const date = new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short", timeZone: "America/Fortaleza" }).format(new Date(workshop.startsAt));
  return (
    <button type="button" className={`workshop-card ${selected ? "selected" : ""}`} onClick={onSelect} disabled={soldOut} aria-pressed={selected}>
      <div className="card-top"><span className={`availability ${soldOut ? "sold-out" : ""}`}>{soldOut ? "Esgotado" : `${remaining} vagas`}</span><span>{date}</span></div>
